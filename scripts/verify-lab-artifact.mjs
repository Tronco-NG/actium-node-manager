...
assert.ok(
  supervisorFiles.some((file) => /actium-node-supervisor-0\.5\.14/u.test(file)),
  "el release set debe incluir Supervisor 0.5.14",
);
...
function inspectWindowsMsi(msiPath, lab, payloadManifest) {
  ...
  assert.equal(lab.bundle?.windows?.wix?.version, "0.7.0.26");
...
function inspectWindowsNsis(exePath, lab, payloadManifest) {
  ...
  assert.equal(lab.version, "0.7.0-lab.26");
  rmSync(extractRoot, { recursive: true, force: true });
}
...
function inspectSupervisorPayload(supervisorFiles, payloadManifest, productSource) {
  const archive = supervisorFiles.find((file) =>
    /actium-node-supervisor-0\.5\.14/u.test(file) && /\.(?:tar\.gz|tgz|zip)$/iu.test(file),
  );
  if (!archive) {
    throw new Error("artefacto Supervisor 0.5.14 ausente o no extraible");
  }
  ...
  const readme = files.find((file) => {
    ...
    return /Actium Node Supervisor 0\.5\.14/u.test(contents);
  });
  assert.ok(readme, "Supervisor debe incluir README de identidad 0.5.14");
  assert.match(readFileSync(readme, "utf8"), /Actium Node Supervisor 0\.5\.14/u);
  if (platform === "windows") {
    assert.ok(
      windowsTemplate,
