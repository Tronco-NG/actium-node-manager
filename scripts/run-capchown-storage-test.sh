...
exec sudo capsh --drop=cap_dac_override,cap_dac_read_search,cap_fowner -- -c "$capsh_cmd storage_site_core_recupera_retry_parcial_sin_dac_adicional --exact && $capsh_cmd storage_agent_recupera_retry_parcial_sin_dac_ni_fowner --exact && $capsh_cmd storage_agent_rechaza_symlink_y_no_sigue_al_objetivo --exact && $capsh_cmd storage_agent_rechaza_fifo_y_no_lo_promueve --exact && $capsh_cmd storage_agent_rechaza_socket_y_no_lo_promueve --exact && $capsh_cmd storage_agent_no_confunde_runtime_json_0600_con_ausente --exact && $capsh_cmd storage_agent_copy_legacy_migra_archivo_faltante --exact && $capsh_cmd storage_agent_restricted_supervisor_falla_lectura_host_directo --exact && $capsh_cmd storage_radio_saf_recupera_retry_parcial_sin_dac --exact && $capsh_cmd storage_fabric_nats_recupera_sin_dac --exact && $capsh_cmd storage_runtime_unit_rechaza_symlink_en_hijo --exact && $capsh_cmd storage_agent_toctou_cerrado_tras_recuperar_root --exact"
