...
assert.ok(
  supervisorFiles.some((file) => /actium-node-supervisor-0\.5\.11/u.test(file)),
  "el release set debe incluir Supervisor 0.5.11",
);
...
function inspectWindowsMsi(msiPath, lab, payloadManifest) {
  ...
  assert.equal(lab.bundle?.windows?.wix?.version, "0.7.0.23");
...
function inspectSupervisorPayload(supervisorFiles, payloadManifest, productSource) {
  const archive = supervisorFiles.find((file) =>
    /actium-node-supervisor-0\.5\.11/u.test(file) && /\.(?:tar\.gz|tgz|zip)$/iu.test(file),
  );
  if (!archive) {
    throw new Error("artefacto Supervisor 0.5.11 ausente o no extraible");
  }
  ...
  const readme = files.find((file) => {
    ...
    return /Actium Node Supervisor 0\.5\.11/u.test(contents);
  });
  assert.ok(readme, "Supervisor debe incluir README de identidad 0.5.11");
  assert.match(readFileSync(readme, "utf8"), /Actium Node Supervisor 0\.5\.11/u);
  if (platform === "windows") {
    assert.ok(
      windowsTemplate,
    ...
    assert.match(
      archive.replaceAll("\\", "/"),
      /actium-node-supervisor-0\.5\.11-lab-windows/u,
    );
  } else {
  ...
  assert.match(productSource, /NODE_SUPERVISOR_VERSION: &str = "0\.5\.11"/u);
  const ipc = readFileSync(resolve(installerRoot, "src-tauri/actium-node-core/src/ipc.rs"), "utf8");
  assert.match(ipc, /SUPERVISOR_VERSION: &str = "0\.5\.11"/u);
