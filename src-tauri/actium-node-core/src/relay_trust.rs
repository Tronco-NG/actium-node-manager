...
pub trait RelayTrustProvider {
    fn resolve_canonical_host(&self, host_id: &str) -> Result<Option<CanonicalHostTrust>, String>;
}
...
impl SnapshotRelayTrustProvider {
    pub fn from_verified_snapshots(snapshots: Vec<RelayTrustSnapshotV1>) -> Result<Self, String> {
        ...
        for snapshot in snapshots {
            let host_id = snapshot.body.scope.host_id.clone();
            if hosts.iter().any(|host: &CanonicalHostTrust| host.host_id == host_id) {
                return Err("R3_CLIENT_SCOPE_RESOLUTION_AMBIGUOUS".to_string());
            }
            hosts.push(CanonicalHostTrust {
                host_id,
...
impl RelayTrustProvider for SnapshotRelayTrustProvider {
    fn resolve_canonical_host(&self, host_id: &str) -> Result<Option<CanonicalHostTrust>, String> {
        ...
        let matches: Vec<_> = self
            .hosts
            .iter()
            .filter(|host| host.host_id == host_id)
            .cloned()
            .collect();
        match matches.len() {
            0 => Ok(None),
...
impl RelayTrustProvider for UnavailableRelayTrustProvider {
    fn resolve_canonical_host(&self, _host_id: &str) -> Result<Option<CanonicalHostTrust>, String> {
...
#[serde(rename_all = "camelCase")]
struct SnapshotSigningEnvelope<'a> {
    domain: &'a str,
    payload: &'a RelayTrustSnapshotUnsignedV1,
}
...
pub fn publish_relay_trust_snapshot(
    ...
    let envelope = SnapshotSigningEnvelope {
        domain: RELAY_TRUST_SNAPSHOT_DOMAIN,
        payload: &body,
    };
    let signature = signer.sign_canonical_value(&serde_json::to_value(envelope).map_err(|error| error.to_string())?)?;
...
impl RelayTrustSnapshotVerifier {
    ...
    pub fn verify(
        ...
        let expected_identity = public_identity_from_key(&snapshot.body.issuer_public_key)?;
...
fn verify_snapshot_signature(snapshot: &RelayTrustSnapshotV1) -> Result<(), String> {
    let raw = URL_SAFE_NO_PAD
        .decode(snapshot.body.issuer_public_key.trim())
    ...
    let canonical = canonical_json(&serde_json::to_value(envelope).map_err(|error| error.to_string())?)?;
...
mod tests {
    ...
    fn verifier_accepts_valid_and_rejects_replay_stale_and_bad_signature() {
        ...
        assert_eq!(
            verifier.verify(&snapshot, now()).unwrap().host_id,
            "host-a"
        );
        assert_eq!(
            verifier.verify(&snapshot, now()).unwrap_err(),
        ...
        stale.signature = signer
            .sign_canonical_value(&serde_json::to_value(envelope).unwrap())
            .unwrap();
        let mut stale_verifier = RelayTrustSnapshotVerifier::new(0);
        assert_eq!(
            stale_verifier.verify(&stale, 1_800_000_000).unwrap_err(),
        ...
        assert_eq!(
            sig_verifier.verify(&bad_sig, now()).unwrap_err(),
    ...
    fn revoked_and_unknown_never_become_trusted() {
        ...
        let mut verifier = RelayTrustSnapshotVerifier::new(60);
        assert_eq!(verifier.verify(&snapshot, now()).unwrap_err(), "TRUST_REJECTED");
        let _ = std::fs::remove_dir_all(dir);
    }
    ...
    fn missing_provider_fails_closed() {
        ...
        assert_eq!(
            provider.resolve_canonical_host("host-a").unwrap_err(),
