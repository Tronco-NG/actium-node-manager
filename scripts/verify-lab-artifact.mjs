...
assert.ok(
  supervisorFiles.some((file) => /actium-node-supervisor-0\.5\.15/u.test(file)),
...
function inspectWindowsNsis(exePath, lab, payloadManifest) {
  ...
  assert.equal(lab.version, "0.7.0-lab.27");
  rmSync(extractRoot, { recursive: true, force: true });
}
...
function inspectSupervisorPayload(supervisorFiles, payloadManifest, productSource) {
  ...
  const readme = files.find((file) => {
    ...
    return /Actium Node Supervisor 0\.5\.15/u.test(contents);
  });
  assert.ok(readme, "Supervisor debe incluir README de identidad 0.5.15");
  assert.match(readFileSync(readme, "utf8"), /Actium Node Supervisor 0\.5\.15/u);
  if (platform === "windows") {
    assert.ok(
      windowsTemplate,
    ...
    assert.match(
      archive.replaceAll("\\", "/"),
      /actium-node-supervisor-0\.5\.15-lab-windows/u,
    );
  } else {
    ...
    assert.match(
      archive.replaceAll("\\", "/"),
      /actium-node-supervisor-0\.5\.15-linux/u,
    );
  }
  assert.match(productSource, /NODE_SUPERVISOR_VERSION: &str = "0\.5\.15"/u);
  const ipc = readFileSync(resolve(installerRoot, "src-tauri/actium-node-core/src/ipc.rs"), "utf8");
  assert.match(ipc, /SUPERVISOR_VERSION: &str = "0\.5\.15"/u);
