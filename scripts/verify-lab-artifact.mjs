...
function inspectWindowsMsi(msiPath, lab, payloadManifest) {
  ...
  assert.equal(lab.bundle?.windows?.wix?.version, "0.7.0.29");
...
function inspectWindowsNsis(exePath, lab, payloadManifest) {
  ...
  assert.equal(lab.version, "0.7.0-lab.29");
  rmSync(extractRoot, { recursive: true, force: true });
}
...
function inspectSupervisorPayload(supervisorFiles, payloadManifest, productSource) {
  ...
  if (platform === "windows") {
    ...
    assert.match(
      archive.replaceAll("\\", "/"),
      /actium-node-supervisor-0\.5\.16-lab-windows/u,
    );
  } else {
    ...
    assert.match(
      archive.replaceAll("\\", "/"),
      /actium-node-supervisor-0\.5\.16-linux/u,
    );
  }
  assert.match(productSource, /NODE_SUPERVISOR_VERSION: &str = "0\.5\.16"/u);
  const ipc = readFileSync(resolve(installerRoot, "src-tauri/actium-node-core/src/ipc.rs"), "utf8");
  assert.match(ipc, /SUPERVISOR_VERSION: &str = "0\.5\.16"/u);
