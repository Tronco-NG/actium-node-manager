...
test("el deployment journal modela fases persistentes y recovery explícito", () => {
  ...
  for (const field of [
    "deployment_id", "channel", "artifact_digest", "previous_artifact_digest", "config_digest",
...
test("preflight, stage y runtime comparten migración y contrato tipado", () => {
  assert.match(config, /CURRENT_CONFIG_SCHEMA_VERSION: u32 = 2/);
  ...
  assert.match(trust, /migrate_legacy_stable_metadata/);
});

test("dpkg termina en capa de paquete y nunca hace activation de canales", () => {
  assert.match(packageScript, /package configured; channel deployment is an explicit transaction/);
