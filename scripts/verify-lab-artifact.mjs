...
function inspectWindowsMsi(msiPath, lab, payloadManifest) {
  ...
  assert.equal(lab.bundle?.windows?.wix?.version, "0.7.0.33");
...
function inspectWindowsNsis(exePath, lab, payloadManifest) {
  ...
  assert.equal(lab.version, "0.7.0-lab.33");
  rmSync(extractRoot, { recursive: true, force: true });
}
...
function inspectSupervisorPayload(supervisorFiles, payloadManifest, productSource) {
  ...
  if (platform === "windows") {
    ...
    assert.match(
      archive.replaceAll("\\", "/"),
      /actium-node-supervisor-0\.5\.19-lab-windows/u,
    );
  } else {
    ...
    assert.match(
      archive.replaceAll("\\", "/"),
      /actium-node-supervisor-0\.5\.19-linux/u,
    );
  }
  assert.match(productSource, /NODE_SUPERVISOR_VERSION: &str = "0\.5\.19"/u);
  const ipc = readFileSync(resolve(installerRoot, "src-tauri/actium-node-core/src/ipc.rs"), "utf8");
  assert.match(ipc, /SUPERVISOR_VERSION: &str = "0\.5\.19"/u);
